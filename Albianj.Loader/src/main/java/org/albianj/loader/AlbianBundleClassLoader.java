...
public class AlbianBundleClassLoader extends ClassLoader{
    ...
    protected Class<?> findClass(String name) throws ClassNotFoundException {
        ...
        // step 4 : if not find from lib directory then find from super classloader
        if (cfm == null) {
            Class<?> c = Class.forName(cfm.getFullClassName()); //when use web container such as jetty then use
            if (null != c) {
                return c;
            }
