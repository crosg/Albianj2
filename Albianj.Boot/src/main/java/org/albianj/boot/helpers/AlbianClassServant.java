...
public class AlbianClassServant {
    public static AlbianClassServant Instance = null;

    static {
        if(null == Instance) {
            Instance = new AlbianClassServant();
        }
    }

    protected AlbianClassServant() {
    ...
    public byte[] getClassFileBytes(String rootFolder,String fullClassName){
        ...
        if(!AlbianFileServant.Instance.isFileOrPathExist(classFileName)) {
            return null;
        }
        return AlbianFileServant.Instance.getFileBytes(classFileName);
    }
