...
import org.albianj.keyword.filter.IKeywordFilterService;
 ...
 */
public abstract class FreeKeywordFilterService extends FreeParserService implements IKeywordFilterService {
    ...
    public void loading() throws AlbianServiceException, AlbianParserException {
        ...
        if (Validate.isNullOrEmpty(keyWordSet)) { // loadConf by configurtion
...
//            }
        ...
        if (Validate.isNullOrEmpty(keyWordSet)) {
            AlbianServiceRouter.getLogger().warn(ILoggerService.AlbianRunningLoggerName,
    ...
    private void readSensitiveWordFile(Set<String> set) {
        ...
        try {
            ...
            AlbianServiceRouter.getLogger().warn(ILoggerService.AlbianRunningLoggerName, "read keyword file:%s is fail.",
                    fname);
            return;
        } finally {
            if (null != read) {
